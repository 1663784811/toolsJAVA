...
public class ${operationTools.indexToUpperCase(tableName)}Const {
    ...
    //允许删除条件
    public final static String deleteColumnArr[] = {<#list javaColumns as column >"${operationTools.allToLowerCase(column.name)}"<#if column_index+1 lt javaColumns?size >,</#if></#list>};
    //允许添加字段
    public final static String addColumnArr[] = {<#list javaColumns as column >"${operationTools.allToLowerCase(column.name)}"<#if column_index+1 lt javaColumns?size >,</#if></#list>};
